...
function makeSession(key, session) {
  ...
  const ownedTabIds = session.owned ? [.../* @__PURE__ */ new Set([
    ...(session.ownedTabIds ?? []).filter((id) => Number.isInteger(id)),
  ...
  if (ownedTabIds.size > 0) {
    try {
      ...
      for (const group of allGroups) {
        ...
        if (tabsInGroup.some((tab) => tab.id !== void 0 && ownedTabIds.has(tab.id))) {
          groupsById.set(group.id, group);
        }
      }
    } catch {
...
async function createOwnedTabLeaseUnlocked(leaseKey, initialUrl) {
  ...
  let tab;
  const canReuseInitialTab = initialTabIsAvailable(initialTabId) && (!existingSession?.owned || existingSession.ownedTabIds.length === 0);
...
async function listLiveOwnedTabs(leaseKey, session) {
  ...
  const liveIds = liveTabs.map((tab) => tab.id).filter((id) => id !== void 0);
  const ownedTabsChanged = liveIds.length !== session.ownedTabIds.length || liveIds.some((id, index) => id !== session.ownedTabIds[index]);
  ...
  if (ownedTabsChanged || session.preferredTabId !== previousPreferredTabId || session.windowId !== previousWindowId) await persistRuntimeState();
  return liveTabs;
}
async function setPreferredOwnedTab(leaseKey, tab) {
  const session = automationSessions.get(leaseKey);
  if (!session?.owned || tab.id === void 0 || !session.ownedTabIds.includes(tab.id)) {
    throw new CommandFailure(
      "tab_not_owned",
...
async function resolveTab(tabId, leaseKey, initialUrl) {
  ...
  if (tabId !== void 0) {
    try {
      ...
      const legacyWindowMatch = !!session?.owned && session.ownedTabIds.length === 0 && session.preferredTabId === null && tab.windowId === session.windowId;
  ...
  const preferred = existingSession.preferredTabId === null ? void 0 : liveTabs.find((tab) => tab.id === existingSession.preferredTabId && isDebuggableUrl(tab.url));
...
async function createOwnedSessionTab(leaseKey, url) {
...
async function handleTabs(cmd, leaseKey) {
  ...
  switch (cmd.op) {
    ...
    case "close": {
      ...
      if (cmd.index !== void 0) {
      ...
      } else if (cmd.page !== void 0) {
        const cmdTabId = await resolveCommandTabId(cmd);
        const currentSession = automationSessions.get(leaseKey);
        if (cmdTabId === void 0 || !currentSession?.owned || !currentSession.ownedTabIds.includes(cmdTabId)) {
        ...
        if (!currentSession?.owned || preferredTabId === null || preferredTabId === void 0) {
          return { id: cmd.id, ok: false, error: "No current owned tab to close" };
        }
        try {
          target = await chrome.tabs.get(preferredTabId);
        } catch {
          target = { id: preferredTabId };
        }
      }
      const targetId = target.id;
      const closedPage = await resolveTargetId(targetId).catch(() => void 0);
    ...
    case "select": {
      ...
      if (cmdTabId !== void 0) {
        ...
        if (!session2?.owned || !session2.ownedTabIds.includes(cmdTabId)) {
          return { id: cmd.id, ok: false, errorCode: "tab_not_owned", error: "Page is not owned by this OpenCLI session." };
        }
...
async function removeOwnedTab(tabId) {
...
async function closeOwnedLeaseTab(leaseKey, tabId, reason) {
  return withLeaseMutation(async () => {
    const session = automationSessions.get(leaseKey);
...
async function releaseLeaseUnlocked(leaseKey, reason) {
  ...
  if (session.owned) {
    ...
    const hasOtherOwnedLease = [...automationSessions.entries()].some(
      ([otherLease, otherSession]) => otherLease !== leaseKey && otherSession.owned && getOwnedWindowRole(otherLease) === role
    );
    if (hasOtherOwnedLease) {
      ...
      try {
        ...
        const fallbackIds = new Set(session.ownedTabIds);
        for (const tab of tabs) if (tab.id !== void 0) fallbackIds.add(tab.id);
        const failures = [];
...
async function reconcileTargetLeaseRegistry() {
  ...
  for (const [leaseKey, stored] of Object.entries(registry.leases)) {
    try {
      ...
      const candidateTabIds = owned ? [.../* @__PURE__ */ new Set([
        ...Array.isArray(stored.ownedTabIds) ? stored.ownedTabIds.filter((id) => Number.isInteger(id)) : [],
      ...
      const preferredTab = liveTabs.find((tab) => tab.id === stored.preferredTabId) ?? liveTabs.at(-1);
      ...
      const session = makeSession(leaseKey, {
        ...
        owned,
        preferredTabId: tabId,
        ownedTabIds: owned ? liveTabs.map((tab) => tab.id).filter((id) => id !== void 0) : []
      });
      const timeout = getIdleTimeout(leaseKey);
      automationSessions.set(leaseKey, {
        ...session,
  ...
  try {
    const interactiveLeases = [...automationSessions.entries()].filter(
      ([leaseKey, session]) => session.owned && getOwnedWindowRole(leaseKey) === "interactive"
    );
