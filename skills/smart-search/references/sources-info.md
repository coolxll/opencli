...
- 前提：已安装可选 `feedly` provider，并配置 Feedly `refresh_token` 或 `access_token`；该 provider 不依赖浏览器
- 使用前先运行：`opencli feedly -h` 和 `opencli feedly search -h`
- 需要更完整的鉴权、范围与模板搜索边界时，读取 provider 附带的 `opencli-feedly` skill
...
- 国际新闻优先 `reuters`
- 中文科技资讯优先 `36kr`
- 用户明确要求搜索“我的订阅 / 我的 RSS / Feedly”时，先确认 `feedly` provider 已安装；可用时优先它，不要替换成通用网页搜索
